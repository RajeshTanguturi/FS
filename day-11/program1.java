 ...
 */
 ...
 class solution{
     public static void dfs(int[] large,int y ,List<Integer> curr, int[] count, int idx){
         if( idx == large.length){
             if(curr.size() == y){
                 boolean flag = true;
                 for(int i = 1 ;i < curr.size() ; i++){
                     if (curr.get(i-1 ) > curr.get(i)) flag = false;
                 }
                 if(flag){
                     count[0]++;
                 }
     ...
     public static void main(String args[]){
         ...
         int count[] = new int[]{0};
         dfs(large,y,new ArrayList<>(), count,0);
