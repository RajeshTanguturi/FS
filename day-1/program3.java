...
public class program3 {
    public static int maxLength(int arr[], int maxFrequency, int maxArea) {
        int maxLen = 0;
        int currArea = 0;
        int start = 0;
        ...
        for (int i = 0; i < arr.length; i++) {
            ...
            currArea += arr[i];
              // If frequency of any room exceeds maxFrequency, shrink the window
            while (freqMap.get(arr[i]) > maxFrequency) {
                freqMap.put(arr[start], freqMap.get(arr[start]) - 1);
                currArea -= arr[start];
                start++;
            }
            // If total area exceeds maxArea, shrink window from left
    ...
    public static void main(String[] args) {
        ...
        int n = sc.nextInt();
        int maxFrequency = sc.nextInt();
        int maxArea = sc.nextInt();
        int arr[] = new int[n];
        for (int i = 0; i < n; i++) {
            arr[i] = sc.nextInt();
        }
        System.out.println(maxLength(arr, maxFrequency, maxArea));
    }
}
